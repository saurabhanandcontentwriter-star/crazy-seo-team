...
// Trusted/official feeds are fetched first; AI only rewrites supplied facts.
...
const CATEGORIES = ["SEO", "AI SEO", "Technical SEO", "Google Updates", "ChatGPT SEO", "AI Tools", "Digital Marketing", "AI Automation", "Machine Learning", "Generative AI", "Voice AI", "Business Intelligence", "Technology", "Cybersecurity", "AI Research", "SaaS & Startups"];
const FEEDS = [
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const feedResults = await Promise.all(FEEDS.map(async (feed) => { try { const r = await fetch(feed.url, { headers: { "User-Agent": "CrazySEO-Team-LiveNews/2.1" } }); return r.ok ? parseFeed(await r.text(), feed.name).slice(0, 12) : []; } catch { return []; } }));
    // Only the newest 72 hours enter the live newsroom. This keeps September 2026 breaking stories at the top.
    const candidates = feedResults.flat().filter((x) => { const t = new Date(x.published_at).getTime(); return !Number.isNaN(t) && Date.now() - t < 72 * 60 * 60 * 1000; }).sort((a, b) => new Date(b.published_at).getTime() - new Date(a.published_at).getTime()).slice(0, 30);
    ...
    const prompt = `You are the senior live editor of Crazy SEO Team Newsroom. Date: ${today}. Create concise, trustworthy, humanized stories using ONLY supplied source facts. Prioritize breaking items from the last 24 hours. Cover AI, technology, SEO, Google Search, AI Search, ChatGPT, Gemini, AI agents, cybersecurity, research, SaaS and digital business. Each article must be approximately 100 words of original editorial copy, not copied from the source. Naturally optimize for Google Search and AI search: clear intent, semantic terms, concise sentences, no keyword stuffing. Never invent facts, quotes, statistics, dates, URLs or source names. Preserve the exact source URL. Return ONLY JSON {"articles":[...]}. Each item: {"title":"<=70 chars","meta_title":"<=60 chars","meta_description":"<=158 chars","focus_keyword":"2-4 words","nlp_keywords":["8-12 natural terms"],"category":"allowed category","source":"exact source","source_url":"exact URL","source_published_at":"ISO date if known","summary":"50-80 chars","image_alt":"descriptive alt","reading_minutes":1,"content_html":"about 100 words using <h2>Introduction</h2>, <h2>Key Updates</h2>, <h2>What This Means</h2>, <h2>Expert Take</h2>, <h2>Conclusion</h2>","faqs":[{"q":"...","a":"..."},{"q":"...","a":"..."},{"q":"...","a":"..."}]}. Allowed categories: ${CATEGORIES.join(", ")}. SOURCE FACTS:\n${packet}`;
    ...
    const raw = (await aiRes.json()).choices?.[0]?.message?.content ?? "{}"; const parsed = JSON.parse(raw); const items = (Array.isArray(parsed) ? parsed : parsed.articles ?? []).slice(0, 15).filter((x: any) => x?.title && x?.content_html && x?.source_url);
    ...
    const { data: oldRows } = await supabase.from("news_articles").select("id").order("published_at", { ascending: false }).range(60, 500); if (oldRows?.length) await supabase.from("news_articles").delete().in("id", oldRows.map((r: any) => r.id));
