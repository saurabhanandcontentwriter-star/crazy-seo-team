...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const candidates = feedResults.flat().sort((a, b) => new Date(b.published_at).getTime() - new Date(a.published_at).getTime()).slice(0, 30);
    ...
    const prompt = `You are the senior human editor of Crazy SEO Team Newsroom. Date: ${today}. Create concise, trustworthy, humanized news stories using ONLY the supplied source facts. Cover the most relevant fresh AI, technology, SEO, Google Search, AI Search, cybersecurity, SaaS and digital-business developments. Prefer genuinely recent items and avoid duplicates. Each article must be about 100 words of main editorial copy, not copied from the source. Make it useful for readers and naturally SEO optimized: clear keyword intent, semantic terms, concise sentences, no keyword stuffing, no robotic phrases. Never invent facts, quotes, statistics, dates, URLs or source names. Preserve the exact source URL. Return ONLY JSON {"articles":[...]}. Each item: {"title":"<=70 chars","meta_title":"<=60 chars","meta_description":"<=158 chars","focus_keyword":"2-4 words","nlp_keywords":["8-12 natural terms"],"category":"allowed category","source":"exact source","source_url":"exact URL","source_published_at":"ISO date if known","summary":"50-80 chars","image_alt":"descriptive alt","reading_minutes":1,"content_html":"about 100 words using <h2>Introduction</h2>, <h2>Key Updates</h2>, <h2>What This Means</h2>, <h2>Expert Take</h2>, <h2>Conclusion</h2>","faqs":[{"q":"...","a":"..."},{"q":"...","a":"..."},{"q":"...","a":"..."}]}. Allowed categories: ${CATEGORIES.join(", ")}. SOURCE FACTS:\n${packet}`;
    ...
    const raw = (await aiRes.json()).choices?.[0]?.message?.content ?? "{}"; const parsed = JSON.parse(raw); const items = (Array.isArray(parsed) ? parsed : parsed.articles ?? []).slice(0, 12).filter((x: any) => x?.title && x?.content_html && x?.source_url);
