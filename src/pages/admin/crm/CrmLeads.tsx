...
import {
  Download, FileSpreadsheet, FileText, Filter, Pencil, Plus, Search, Trash2, Users2, Phone, Mail,
} from "lucide-react";
...
import {
  ...
  fetchLeads, fetchTeam, fullPhone, relativeTime, stageLabel, statusTone, updateLead, inr,
} from "@/lib/crm";
...
export default function CrmLeads() {
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter((l) => {
      if (term) {
        const hay = [l.full_name, l.email, l.phone, fullPhone(l), l.company, l.city]
      ...
      if (owner !== ANY) {
        if (owner === UNASSIGNED ? !!l.assigned_to : l.assigned_to !== owner) return false;
      }
    ...
    try {
      if (draft.id) {
        await updateLead(draft.id, {
          full_name: draft.full_name, email: draft.email, phone_country: draft.phone_country || "+91",
          phone: draft.phone, company: draft.company ?? null, city: draft.city ?? null,
          state: draft.state ?? null, country: draft.country ?? null, source: draft.source || "crm_manual",
          service: draft.service || "Free Consultation", status: draft.status || "new",
          score: Number(draft.score) || 0, deal_value: Number(draft.deal_value) || 0,
          assigned_to: draft.assigned_to || null, message: draft.message ?? null,
        });
  ...
  const assign = async (lead: CrmLead, value: string) => {
    ...
    try {
      await updateLead(lead.id, { assigned_to });
    ...
    <div className="space-y-4">
      <GlassCard className="p-4">
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <Input
              ...
              placeholder="Search name, email, mobile, company, city…"
              className="pl-9 rounded-xl"
            />
          </div>
          <Button onClick={() => setDraft({ phone_country: "+91", status: "new", source: "crm_manual", service: "Free Consultation", score: 30 })} className="rounded-xl gap-2">
            <Plus size={16} /> New lead
          </Button>
          <Button variant="outline" className="rounded-xl gap-2" onClick={() => filtered.length ? exportCsv(filtered, team) : toast.error("Nothing to export")}>
            <Download size={15} /> CSV
          </Button>
          <Button variant="outline" className="rounded-xl gap-2" onClick={() => filtered.length ? exportExcel(filtered, team) : toast.error("Nothing to export")}>
            <FileSpreadsheet size={15} /> Excel
          </Button>
          <Button variant="outline" className="rounded-xl gap-2" onClick={() => filtered.length ? exportPdf(filtered, team) : toast.error("Nothing to export")}>
        ...
        <div className="mt-3 grid gap-2 grid-cols-2 md:grid-cols-3 xl:grid-cols-6">
          ...
          <Select value={owner} onValueChange={setOwner}>
            <SelectTrigger className="rounded-xl"><SelectValue placeholder="Owner" /></SelectTrigger>
            <SelectContent>
        ...
        <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
          <Filter size={13} /> {filtered.length} of {leads.length} leads
          {(q || status !== ANY || source !== ANY || service !== ANY || owner !== ANY || from || to) && (
            <button
              className="underline hover:text-foreground"
              onClick={() => { setQ(""); setStatus(ANY); setSource(ANY); setService(ANY); setOwner(ANY); setFrom(""); setTo(""); }}
            >
              Reset
            </button>
          )}
      ...
      <GlassCard className="overflow-hidden">
        {filtered.length === 0 ? (
          <EmptyState
            ...
            hint="Adjust the search or create a new lead to get started."
            action={<Button className="rounded-xl" onClick={() => setDraft({ phone_country: "+91", status: "new", source: "crm_manual", service: "Free Consultation", score: 30 })}>Create lead</Button>}
          />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[980px]">
              <thead className="bg-muted/50 text-xs uppercase tracking-wide text-muted-foreground">
                <tr>
                  {["Lead", "Contact", "Location", "Service", "Owner", "Status", "Score", "Value", "Created", ""].map((h) => (
                ...
                {filtered.map((l, i) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <Link to={`/admin/crm/leads/${l.id}`} className="font-semibold hover:text-primary">{l.full_name}</Link>
                      <p className="text-xs text-muted-foreground">{l.company || "—"}</p>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <a href={`tel:${fullPhone(l)}`} className="inline-flex items-center gap-1 text-xs hover:text-primary"><Phone size={12} />{fullPhone(l)}</a>
                      </div>
                      <a href={`mailto:${l.email}`} className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-primary"><Mail size={12} />{l.email}</a>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <Select value={l.assigned_to ?? UNASSIGNED} onValueChange={(v) => assign(l, v)}>
                        <SelectTrigger className="h-8 rounded-lg text-xs w-[140px]"><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value={UNASSIGNED}>Unassigned</SelectItem>
                          {team.map((t) => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}
                        </SelectContent>
                    ...
                    <td className="px-4 py-3">
                      <Select value={l.status} onValueChange={(v) => setStage(l, v)}>
                        <SelectTrigger className={`h-8 rounded-lg text-xs w-[130px] border ${statusTone(l.status)}`}><SelectValue /></SelectTrigger>
                        <SelectContent>
                          {CRM_STAGES.map((s) => <SelectItem key={s.id} value={s.id}>{s.label}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </td>
                    <td className="px-4 py-3 font-bold tabular-nums">{l.score}</td>
                    <td className="px-4 py-3 text-xs tabular-nums">{inr(l.deal_value ?? 0)}</td>
                    <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">{relativeTime(l.created_at)}</td>
                    <td className="px-4 py-3">
      ...
      <Dialog open={!!draft} onOpenChange={(o) => !o && setDraft(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{draft?.id ? "Edit lead" : "New lead"}</DialogTitle></DialogHeader>
          {draft && (
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Field label="Status">
                <Select value={draft.status ?? "new"} onValueChange={(v) => setDraft({ ...draft, status: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{CRM_STAGES.map((s) => <SelectItem key={s.id} value={s.id}>{s.label}</SelectItem>)}</SelectContent>
                </Select>
              </Field>
              <Field label="Assigned to">
                <Select value={draft.assigned_to ?? UNASSIGNED} onValueChange={(v) => setDraft({ ...draft, assigned_to: v === UNASSIGNED ? null : v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value={UNASSIGNED}>Unassigned</SelectItem>
                    {team.map((t) => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}
              ...
              <div className="sm:col-span-2">
                <Field label="Notes"><Textarea rows={3} value={draft.message ?? ""} onChange={(e) => setDraft({ ...draft, message: e.target.value })} /></Field>
              </div>
            </div>
          )}
          <DialogFooter>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {deleting?.full_name} and all related activities and follow-ups will be permanently removed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
