...
import { Clock3, LogIn, LogOut, RefreshCw, UsersRound } from "lucide-react";
...
export default function CrmAttendancePanel() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [{ data: userData, error: userError }, { data: team, error: teamError }, { data: attendance, error: attendanceError }] = await Promise.all([
  ...
  return <GlassCard className="p-5">
    ...
    <div className="mt-5 overflow-x-auto"><div className="min-w-[760px]"><div className="grid grid-cols-[1.4fr_1fr_1fr_1fr_1fr] gap-3 border-b px-3 pb-2 text-[10px] font-bold uppercase tracking-wide text-muted-foreground"><span>Member</span><span>Status</span><span>Punch In</span><span>Punch Out</span><span>Working Time</span></div>
      <div className="divide-y">{rows.map(r => <div key={r.member.id} className="grid grid-cols-[1.4fr_1fr_1fr_1fr_1fr] items-center gap-3 px-3 py-3"><div className="flex items-center gap-2 min-w-0">{r.member.photo_url ? <img src={r.member.photo_url} alt={r.member.name} className="h-9 w-9 rounded-xl object-cover" /> : <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-blue-600 to-violet-600 text-xs font-black text-white">{r.member.name.charAt(0).toUpperCase()}</span>}<div className="min-w-0"><p className="truncate text-sm font-bold">{r.member.name}</p><p className="truncate text-[10px] text-muted-foreground">{r.member.login_id ?? "Team member"}</p></div></div><span className={"inline-flex w-fit rounded-lg border px-2 py-1 text-[10px] font-bold " + statusClass(r.punch_in ? r : null)}>{statusLabel(r.punch_in ? r : null)}</span><span className="text-xs font-semibold">{formatClock(r.punch_in)}</span><span className="text-xs font-semibold">{formatClock(r.punch_out)}</span><span className="text-xs font-semibold">{formatDuration(r.status === "punched_in" && r.punch_in ? Math.max(0, Math.floor((Date.now() - new Date(r.punch_in).getTime()) / 1000)) : Number(r.total_seconds || 0))}</span></div>)}{!rows.length && !loading && <div className="px-3 py-8 text-center text-sm text-muted-foreground"><UsersRound size={18} className="mx-auto mb-2" />No active CRM members found.</div>}</div></div>
    </div>
  </GlassCard>;
}