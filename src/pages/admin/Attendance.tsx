...
type Row={id:string;user_id:string;email:string;work_date:string;punch_in:string|null;punch_out:string|null;total_seconds:number;status:"punched_in"|"punched_out";work_status:"red"|"green"};
...
export default function Attendance(){
 ...
 return <div className="space-y-6">
  ...
  <Card><CardHeader><CardTitle className="flex items-center gap-2"><CalendarDays size={18}/> Attendance records <span className="text-sm font-normal text-muted-foreground">({filtered.length})</span></CardTitle></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left text-xs text-muted-foreground"><th className="p-4">Date</th><th className="p-4">Employee</th><th className="p-4">Punch in</th><th className="p-4">Punch out</th><th className="p-4">Work time</th><th className="p-4">9 Hour Status</th><th className="p-4">Status</th></tr></thead><tbody>{loading?<tr><td colSpan={7} className="p-10 text-center text-muted-foreground">Loading attendance…</td></tr>:filtered.length?filtered.map(r=><tr key={r.id} className={"border-b last:border-0 hover:bg-muted/30 " + (r.work_status==="green" ? "bg-emerald-500/5" : "bg-red-500/5")}<td className="p-4 font-medium">{r.work_date}</td><td className="p-4"><div className="font-semibold">{employeeName(r.email)}</div></td><td className="p-4">{formatClock(r.punch_in)}</td><td className="p-4">{formatClock(r.punch_out)}</td><td className="p-4 font-mono text-xs">{formatDuration(r.total_seconds)}</td><td className="p-4"><Badge className={r.work_status==="green" ? "bg-emerald-500/10 text-emerald-700 border-emerald-500/20" : "bg-red-500/10 text-red-700 border-red-500/20"}>{r.work_status==="green" ? "9+ Hours" : "Under 9 Hours"}</Badge></td><td className="p-4"><Badge variant={r.status==="punched_in"?"default":"outline"}>{r.status==="punched_in"?"Punched in":"Completed"}</Badge></td></tr>):<tr><td colSpan={7} className="p-12 text-center text-muted-foreground">No attendance records for this filter.</td></tr>}</tbody></table></div></CardContent></Card>
 </div>
}
