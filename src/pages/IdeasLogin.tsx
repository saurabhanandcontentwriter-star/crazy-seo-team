...
export default function IdeasLogin(){
  ...
  const [resendCooldown,setResendCooldown]=useState(0),[otpFailures,setOtpFailures]=useState(0),[lockedUntil,setLockedUntil]=useState(0);

  useEffect(()=>{
    const mail=email.trim().toLowerCase();
    if(!mail)return;
    const raw=localStorage.getItem(`ideas_login_otp_lock_${mail}`);
    if(raw){const until=Number(raw);if(until>Date.now())setLockedUntil(until);else localStorage.removeItem(`ideas_login_otp_lock_${mail}`);}
    setOtpFailures(Number(localStorage.getItem(`ideas_login_otp_failures_${mail}`)||0));
  },[email]);
  ...
  const sendCode=async()=>{
    ...
    if(lockedUntil>Date.now())return toast.error(`Login is locked for 3 hours. Try again after ${new Date(lockedUntil).toLocaleString()}.`);
  ...
  const verify=async()=>{
    ...
    try{
      ...
      if(error){
        ...
        if(next>=3){const until=Date.now()+3*60*60*1000;localStorage.setItem(`ideas_login_otp_lock_${mail}`,String(until));localStorage.setItem(`ideas_login_otp_failures_${mail}`,String(next));setLockedUntil(until);setOtpFailures(next);throw new Error("3 incorrect verification attempts. Login is locked for 3 hours.");}
        setOtpFailures(next);localStorage.setItem(`ideas_login_otp_failures_${mail}`,String(next));throw new Error(`Invalid verification code. ${3-next} attempt${3-next===1?"":"s"} remaining.`);
      }
      const mail=email.trim().toLowerCase();
      localStorage.removeItem(`ideas_login_otp_failures_${mail}`);localStorage.removeItem(`ideas_login_otp_lock_${mail}`);setOtpFailures(0);setLockedUntil(0);
