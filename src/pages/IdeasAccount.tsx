...
if(direct){try{const p=JSON.parse(direct);if(p?.public_id){setCreatedId(p.public_id);setEmail(p.email||"");setChecking(false);return;}}catch{sessionStorage.removeItem(sessionKey)}}
  ...
  const permanentlyDelete=async()=>{
    if(!window.confirm("PERMANENT DELETE: This will permanently delete your Ideas account and linked Ideas data. Continue?"))return;
  ...
  if(banned)return <div className="min-h-screen bg-background"><div className="container mx-auto max-w-2xl px-4 py-8 md:py-14"><Card className="rounded-[28px] border-destructive/30"><CardContent className="space-y-5 p-6 md:p-8"><div className="flex items-start gap-3"><AlertTriangle className="mt-1 text-destructive"/><div><h1 className="text-3xl font-black">Ideas Account Banned</h1><p className="mt-2 text-muted-foreground">Your Ideas account is currently banned.</p>{banned.reason&&<p className="mt-3 rounded-xl bg-muted p-3 text-sm"><strong>Reason:</strong> {banned.reason}</p>}</div></div><Button variant="destructive" onClick={permanentlyDelete} disabled={deleting}>{deleting?<Loader2 className="mr-2 size-4 animate-spin"/>:<Trash2 className="mr-2 size-4"/>}{deleting?"Deleting Permanently...":"Delete Account Permanently"}</Button><Button variant="ghost" className="w-full" onClick={()=>nav("/ideas")}>Back to Ideas</Button></CardContent></Card></div></div>;
  if(createdId)return <div className="min-h-screen bg-background"><div className="container mx-auto max-w-2xl px-4 py-8 md:py-14"><Card className="overflow-hidden rounded-[28px]"><div className="bg-gradient-to-r from-blue-600 via-violet-600 to-fuchsia-600 p-7 text-white"><h1 className="text-3xl font-black">Your Ideas ID is Ready</h1><p className="mt-2 text-white/80">No OTP, no verification link. Your Ideas account is ready.</p></div><CardContent className="space-y-5 p-6 md:p-8"><div className="rounded-2xl border-2 border-primary/30 bg-primary/5 p-5 text-center"><p className="text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground">Your Unique Ideas ID</p><p className="mt-3 break-all text-3xl font-black tracking-wider">{createdId}</p></div><div className="grid gap-3 sm:grid-cols-2"><Button variant="outline" className="rounded-xl" onClick={async()=>{await navigator.clipboard.writeText(createdId);toast.success("Ideas ID copied.");}}>Copy Ideas ID</Button><Button className="rounded-xl" onClick={()=>nav("/ideas/profile/me",{replace:true})}>Continue to Profile</Button></div></CardContent></Card></div></div>;

  return <div className="min-h-screen bg-background"><div className="container mx-auto max-w-2xl px-4 py-8 md:py-14"><Card className="overflow-hidden rounded-[28px]"><div className="bg-gradient-to-r from-blue-600 via-violet-600 to-fuchsia-600 p-7 text-white"><div className="flex items-center gap-3"><span className="rounded-2xl bg-white/15 p-3"><UserPlus/></span><div><h1 className="text-3xl font-black">Create Ideas Account</h1><p className="mt-1 text-white/80">Create your Ideas profile directly with Gmail.</p></div></div></div><CardContent className="space-y-5 p-6 md:p-8"><div className="grid gap-4 md:grid-cols-3"><div className="grid gap-2"><Label>First Name *</Label><Input value={firstName} onChange={e=>setFirstName(e.target.value)} placeholder="First name"/></div><div className="grid gap-2"><Label>Middle Name</Label><Input value={middleName} onChange={e=>setMiddleName(e.target.value)} placeholder="Middle name"/></div><div className="grid gap-2"><Label>Last Name *</Label><Input value={lastName} onChange={e=>setLastName(e.target.value)} placeholder="Last name"/></div></div><div className="grid gap-4 md:grid-cols-2"><div className="grid gap-2"><Label>State *</Label><Input value={state} onChange={e=>setState(e.target.value)} placeholder="State"/></div><div className="grid gap-2"><Label>Country *</Label><Input value={country} onChange={e=>setCountry(e.target.value)} placeholder="Country"/></div></div><div className="grid gap-2"><Label>Gmail *</Label><Input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@gmail.com"/></div><div className="rounded-2xl border bg-muted/30 p-4 text-sm text-muted-foreground"><ShieldCheck className="mr-2 inline size-4 text-primary"/>No OTP and no Gmail verification link.</div><Button className="w-full rounded-xl" onClick={createAccount} disabled={saving}>{saving?<Loader2 className="mr-2 size-4 animate-spin"/>:<ShieldCheck className="mr-2 size-4"/>}Create Ideas Account</Button><Button variant="ghost" className="w-full" onClick={()=>nav("/ideas")}>Back to Ideas</Button></CardContent></Card></div></div>;
}
