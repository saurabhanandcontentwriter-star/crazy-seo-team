...
export default function IdeasCreate() {
 ...
 const editorRef=useRef<HTMLDivElement|null>(null); const [mentionQuery,setMentionQuery]=useState(""); const [mentionUsers,setMentionUsers]=useState<any[]>([]); const [showMentions,setShowMentions]=useState(false); const [coverFile,setCoverFile]=useState<File|null>(null); const [imageAlt,setImageAlt]=useState(""); const [eventStart,setEventStart]=useState(""); const [eventEnd,setEventEnd]=useState(""); const [eventLocation,setEventLocation]=useState(""); const [eventUrl,setEventUrl]=useState(""); const [eventMaxAttendees,setEventMaxAttendees]=useState(""); const [saving,setSaving]=useState(false);
 ...
 const handleEditorInput=(e:React.FormEvent<HTMLDivElement>)=>{
   const el=e.currentTarget; setContent(el.innerHTML);
   const text=el.innerText||""; const match=text.match(/(^|\s)@([A-Za-z0-9._-]{1,30})$/);
   if(match){const q=match[2];setMentionQuery(q);setShowMentions(true);supabase.from("idea_profiles").select("user_id,display_name,public_id").ilike("display_name",q+"%").limit(6).then(({data})=>setMentionUsers(data||[]));}
   else{setShowMentions(false);setMentionUsers([]);}
 };
 const insertMention=(u:any)=>{
   const el=editorRef.current;if(!el)return;
   const text=el.innerText||"";const match=text.match(/(^|\s)@[A-Za-z0-9._-]{1,30}$/);
   if(match){const before=text.slice(0,text.length-match[0].length);el.innerText=before+(before&&!/\s$/.test(before)?" ":"")+"@"+u.display_name+" ";}
   el.focus();setContent(el.innerHTML);setShowMentions(false);setMentionUsers([]);setMentionQuery("");
 };
        ...
        <CardContent className="p-5 md:p-8">
          ...
          <div className="grid gap-6">
            <div className="grid gap-6 md:grid-cols-3">
              ...
              <div className="grid gap-2"><Label>Subject</Label><Select value={subject} onValueChange={v => setSubject(v)}><SelectTrigger className="h-12 rounded-xl"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Technology">Technology</SelectItem><SelectItem value="AI">AI</SelectItem><SelectItem value="SEO">SEO</SelectItem><SelectItem value="Marketing">Marketing</SelectItem><SelectItem value="Business">Business</SelectItem><SelectItem value="Finance">Finance</SelectItem><SelectItem value="Economics">Economics</SelectItem><SelectItem value="Science">Science</SelectItem><SelectItem value="Education">Education</SelectItem><SelectItem value="Health">Health</SelectItem><SelectItem value="Travel">Travel</SelectItem><SelectItem value="Design">Design</SelectItem><SelectItem value="Development">Development</SelectItem><SelectItem value="Startups">Startups</SelectItem><SelectItem value="SaaS">SaaS</SelectItem><SelectItem value="Productivity">Productivity</SelectItem><SelectItem value="Career">Career</SelectItem><SelectItem value="Content">Content</SelectItem><SelectItem value="Data">Data</SelectItem><SelectItem value="Other">Other</SelectItem></SelectContent></Select></div>
            </div>
