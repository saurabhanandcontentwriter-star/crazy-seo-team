...
export default function IdeasCreate() {
 ...
 const [subject,setSubject]=useState<"Tech"|"AI"|"SEO"|"Travel"|"Science"|"Economics"|"Other">("Tech"); const [postType,setPostType]=useState<"post"|"question"|"event">("post"); const [visibility,setVisibility]=useState<"public"|"friends">("public");
 ...
 const submit=async()=>{
   ...
   try{
     ...
     const profile=await supabase.from("idea_profiles").select("user_id,first_name,middle_name,last_name,state,country,account_status,banned_until").eq("user_id",user.id).maybeSingle();
     ...
     const{data:guard,error:guardError}=await supabase.functions.invoke("idea-content-guard",{body:{
       name:publicName,subject,title:title.trim(),content:richContent,location:location.trim(),deviceType:device,postType,visibility,
        ...
        <CardContent className="p-5 md:p-8">
          ...
          <div className="grid gap-6">
            <div className="grid gap-6 md:grid-cols-3">
              ...
              <div className="grid gap-2"><Label>Subject</Label><Select value={subject} onValueChange={v => setSubject(v as any)}><SelectTrigger className="h-12 rounded-xl"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Tech">Tech</SelectItem><SelectItem value="AI">AI</SelectItem><SelectItem value="SEO">SEO</SelectItem><SelectItem value="Travel">Travel</SelectItem><SelectItem value="Science">Science</SelectItem><SelectItem value="Economics">Economics</SelectItem><SelectItem value="Other">Other</SelectItem></SelectContent></Select></div>
            </div>
