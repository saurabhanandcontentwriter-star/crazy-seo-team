...
export default function IdeasProfile(){
 ...
 return (
  <div className="min-h-screen bg-muted/20">
   <div className="mx-auto max-w-5xl px-0 sm:px-4 md:py-6">
    <div className="flex items-center justify-between px-4 py-3 sm:px-0">
      <Button variant="ghost" size="sm" onClick={()=>nav("/anvya")}><ArrowLeft className="mr-2 size-4"/>ANVYA</Button>
      <div className="text-xs font-semibold tracking-[0.18em] text-muted-foreground">PROFILE</div>
    </div>
    <Card className="overflow-hidden rounded-none border-x-0 shadow-sm sm:rounded-3xl sm:border-x">
      <div className="relative h-52 sm:h-64 md:h-72">
        {p.cover_url ? <img src={p.cover_url} className="size-full object-cover" alt="Profile cover"/> : <div className="size-full bg-gradient-to-br from-slate-900 via-violet-700 to-fuchsia-500"/>}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/5 to-black/20"/>
        {me===p.user_id&&edit&&<label className="absolute right-4 top-4 cursor-pointer rounded-full border border-white/20 bg-black/60 px-4 py-2 text-xs font-bold text-white backdrop-blur-md hover:bg-black/75"><Camera className="mr-2 inline size-4"/>Change cover<input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={e=>setCoverFile(e.target.files?.[0]||null)}/></label>}
      </div>
      <CardContent className="relative px-4 pb-6 sm:px-7 md:px-10">
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div className="relative -mt-16 flex min-w-0 items-end gap-4 sm:-mt-20">
            <div className={"relative size-28 shrink-0 overflow-hidden rounded-full border-4 border-background bg-muted shadow-2xl sm:size-36 "+(hasActiveStory?"ring-4 ring-pink-500 ring-offset-2 ring-offset-background":"")}>
              ...
              {me===p.user_id&&edit&&<label className="absolute inset-x-0 bottom-0 cursor-pointer bg-black/75 py-2 text-center text-[11px] font-bold text-white">Change<input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={e=>setAvatarFile(e.target.files?.[0]||null)}/></label>}
            </div>
            <div className="pb-1">
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="text-2xl font-black leading-tight sm:text-3xl">{p.display_name}</h1>
                {p.verified&&<Badge className="rounded-full bg-blue-600 text-white">✓</Badge>}
              </div>
              <p className="text-sm text-muted-foreground">@{p.profile_slug||p.public_id||"anvya-member"}</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            {me===p.user_id?<Button className="rounded-xl" variant="outline" onClick={()=>setEdit(v=>!v)}>{edit?"Cancel":"Edit Profile"}</Button>:<>
              <Button className="rounded-xl px-6" onClick={toggleFollow}>{following?<UserCheck className="mr-2 size-4"/>:<UserPlus className="mr-2 size-4"/>}{following?"Following":"Follow"}</Button>
              <Button className="rounded-xl" variant="outline" onClick={()=>{sessionStorage.setItem("ideas_message_target",JSON.stringify({user_id:p.user_id,display_name:p.display_name,avatar_url:p.avatar_url||null,public_id:p.public_id||null}));setTab("messages")}}><MessageCircle className="mr-2 size-4"/>Message</Button>
            </>}
          </div>
        </div>
        <div className="mt-6 grid grid-cols-3 overflow-hidden rounded-2xl border bg-background">
          <button className="border-r py-4 transition hover:bg-muted/40" onClick={()=>setTab("posts")}><b className="block text-xl sm:text-2xl">{posts.length}</b><span className="text-xs text-muted-foreground">Posts</span></button>
          <button className="border-r py-4 transition hover:bg-muted/40" onClick={()=>setTab("followers")}><b className="block text-xl sm:text-2xl">{followerCount}</b><span className="text-xs text-muted-foreground">Followers</span></button>
          <button className="py-4 transition hover:bg-muted/40" onClick={()=>setTab("following")}><b className="block text-xl sm:text-2xl">{followingCount}</b><span className="text-xs text-muted-foreground">Following</span></button>
        </div>
        <div className="mt-6 space-y-3">
          {p.bio&&<p className="max-w-2xl whitespace-pre-line text-sm leading-6">{p.bio}</p>}
          <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-muted-foreground">
            <span>📍 {[p.location,p.state,p.country].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i).join(", ")||"ANVYA Community"}</span>
            {p.working&&<span>💼 {p.working}</span>}{p.company&&<span>🏢 {p.company}</span>}{p.education&&<span>🎓 {p.education}</span>}
          </div>
          {social.length>0&&<div className="flex flex-wrap gap-2">{social.map(([label,url])=><a key={label} href={String(url)} target="_blank" rel="noreferrer" className="rounded-full border px-3 py-1 text-xs font-semibold transition hover:bg-muted">{label}</a>)}</div>}
        </div>
        <div className="mt-6 border-t pt-5"><IdeasStoryHighlights profileUserId={p.user_id} avatarUrl={p.avatar_url} displayName={p.display_name} isOwner={me===p.user_id}/></div>
        <div className="mt-6 flex overflow-x-auto border-t">
          {([["posts","Posts",FileText],["reshares","Reshares",Activity],["messages","Messages",MessageCircle],["friends","Friends",Users]] as const).map(([k,label,I])=><button key={k} onClick={()=>setTab(k)} className={"flex min-w-fit items-center gap-2 border-b-2 px-5 py-3 text-sm font-semibold transition "+(tab===k?"border-primary text-foreground":"border-transparent text-muted-foreground hover:text-foreground")}><I className="size-4"/>{label}</button>)}
        </div>
      </CardContent>
    </Card>
...
function PostCard({x,resharedByProfile=false}:{x:Post;resharedByProfile?:boolean}){
 ...
 const bookmark=()=>{const b=JSON.parse(localStorage.getItem("ideas-bookmarks")||"[]");const next=bookmarked?b.filter((id:string)=>id!==x.id):[...b,x.id];localStorage.setItem("ideas-bookmarks",JSON.stringify(next));setBookmarked(!bookmarked);toast.success(bookmarked?"Removed from bookmarks":"Saved to bookmarks.")};
