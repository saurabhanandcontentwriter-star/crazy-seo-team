...
export default function IdeasProfile(){
 ...
 if(!p)return <div className="container mx-auto max-w-3xl px-4 py-12"><Card><CardContent className="p-10 text-center">Profile not found.</CardContent><</div>;
 return (
  <div className="min-h-screen bg-background">
   <div className="container mx-auto max-w-5xl px-4 py-6 md:py-10">
    ...
    <
    {me===p.user_id&&requesters.length>0&&<Card className="mt-5"><CardContent className="p-6"><h3 className="mb-4 text-lg font-bold">Friend Requests</h3>{requesters.map(r=><div key={r.user_id} className="flex items-center gap-3 border-b py-3"><div className="size-10 overflow-hidden rounded-full bg-muted">{r.avatar_url?<img src={r.avatar_url} className="size-full object-cover" alt={r.display_name}/>:<UserCircle2 className="size-full p-2"/>}</div><div className="flex-1"><p className="font-bold">{r.display_name}</p><p className="text-xs text-muted-foreground">{r.location||"ANVYA Community"}</p></div><Button size="sm" onClick={()=>respondFriend(r.user_id,"accepted")}><UserCheck className="mr-1 size-4"/>Accept</Button><Button size="sm" variant="outline" onClick={()=>respondFriend(r.user_id,"rejected")}>Reject</Button></div>)}</CardContent><}
...
function ProfileProgress({userId,posts,level,points}:{userId:string;posts:number;level:number;points:number}){
 ...
 return <div className="mt-5 grid gap-4 md:grid-cols-3">
  <Card><CardContent className="p-5"><p className="text-xs uppercase text-muted-foreground">Community Level</p><p className="mt-1 text-2xl font-black">Level {level}</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full bg-primary" style={{width:Math.min(100,((points%100)/100)*100)+"%"}}/></div><p className="mt-2 text-xs text-muted-foreground">{points} reputation points • keep contributing to unlock stages</p></CardContent><
  <Card><CardContent className="p-5"><p className="text-xs uppercase text-muted-foreground">Badges & Leadership</p><div className="mt-3 flex flex-wrap gap-2">{badges.length?badges.map(b=><Badge key={b.id}>🏅 {b.badge_name}</Badge>):<Badge variant="outline">🌱 Community Starter</Badge>}</div><p className="mt-3 text-xs text-muted-foreground">Stages: Starter → Contributor → Leader → Community Builder</p><div className="mt-4 rounded-xl border bg-muted/20 p-3"><p className="text-xs font-bold uppercase text-muted-foreground">Post Achievements</p>{achievements.length?<div className="mt-2 space-y-2">{achievements.slice(0,8).map(a=><div key={a.id} className="flex items-center justify-between rounded-lg border bg-background p-2"><span className="text-sm">🏅 {a.achievement_name}</span><Badge variant="outline">+{a.points_awarded} pts</Badge></div>)}</div>:<p className="mt-2 text-xs text-muted-foreground">Publish approved posts to earn achievements and points.</p>}</div></CardContent><
  <Card><CardContent className="p-5"><p className="text-xs uppercase text-muted-foreground">Communities</p>{communities.length?<div className="mt-2 space-y-2">{communities.slice(0,4).map(c=><div key={c.id} className="flex items-center justify-between rounded-lg border p-2"><span className="font-medium">{c.name}</span><Badge variant="outline">{c.role}</Badge></div>)}</div>:<p className="mt-3 text-sm text-muted-foreground">No communities joined yet.</p>}<Button size="sm" className="mt-3" onClick={()=>location.href="/anvya/communities"}><Users className="mr-2 size-4"/>Explore Communities</Button></CardContent><
 </div>;
}
