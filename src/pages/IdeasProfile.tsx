...
export default function IdeasProfile(){
 ...
 const [verificationResult,postsResult,resharesResult,incomingResult]=await Promise.all([
  ...
  supabase.from("idea_posts").select("id,user_id,display_name,profile_image_url,title,content,post_type,visibility,subject,tags,image_url,created_at,status").eq("user_id",id).in("status",user&&id===user.id?["approved","pending","rejected"]:["approved"]).order("created_at",{ascending:false}),
