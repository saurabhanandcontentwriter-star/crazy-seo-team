...
import {ArrowLeft,UserPlus,UserCheck,Users,FileText,Activity,HelpCircle,Save,ExternalLink,Loader2,UserCircle2,Camera,UserRoundPlus} from "lucide-react";
    ...
    <Card className="mt-4 overflow-hidden rounded-[28px]">
     ...
     <CardContent className="relative p-6">
      <div className="-mt-20 flex flex-col gap-4 md:flex-row md:items-end">
       ...
       <div className="flex-1"><h1 className="text-3xl font-black">{p.display_name}</h1><p className="text-sm text-muted-foreground">{p.location||"Ideas Community member"}</p></div>
       {me===p.user_id?<Button onClick={()=>setEdit(!edit)}>{edit?"Cancel":"Edit Profile"}</Button>:<div className="flex flex-wrap gap-2"><Button onClick={toggleFollow} variant={following?"outline":"default"}>{following?<UserCheck className="mr-2 size-4"/>:<UserPlus className="mr-2 size-4"/>}{following?"Unfollow":"Follow"}</Button><Button variant="outline" onClick={friend} disabled={friendStatus==="accepted"||friendStatus==="pending"}>{friendStatus==="accepted"?<UserCheck className="mr-2 size-4"/>:<Users className="mr-2 size-4"/>}{friendStatus==="accepted"?"Friends":friendStatus==="pending"?"Request Sent":"Add Friend"}</Button></div>}
      </div>
...
function PostCard({x}:{x:Post}){
 return <Card><CardContent className="p-5"><div className="mb-2 flex gap-2"><Badge>{x.post_type==="question"?"Question":x.subject}</Badge><Badge variant="outline">{x.visibility==="friends"?"👥 Friends Only":"🌍 Public"}</Badge></div><h2 className="text-xl font-bold">{x.title}</h2><p className="mt-2 whitespace-pre-wrap text-muted-foreground">{x.content}</p>{x.image_url&&<img src={x.image_url} className="mt-4 max-h-96 rounded-2xl object-cover" alt={x.title}/>}</CardContent></Card>;
}
