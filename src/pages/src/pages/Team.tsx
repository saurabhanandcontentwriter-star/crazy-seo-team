  ...
  <div className="min-h-screen bg-background">
    ...
    <main className="pt-16">
      ...
      <section className="py-20 bg-muted/30"><div className="container mx-auto max-w-6xl px-4"><div className="text-center mb-12"><p className="text-sm font-bold uppercase tracking-[0.16em] text-primary">One Platform</p><h2 className="mt-3 text-3xl font-black md:text-5xl">🧩 One Platform. Different Possibilities.</h2></div><div className="grid gap-6 md:grid-cols-3">
        {[["🔎 Crazy SEO Team","Grow your online visibility.","SEO, AI SEO, AEO, GEO, digital marketing, automation and AI technology solutions.","/services","Explore Crazy SEO Team"],["📦 Product Classified","Put your product or service online.","Create listings and showcase what you sell or provide.","/post-ad","Create a Classified Post"],["🎨 Meet ANVYA","Turn your ideas into your creator journey.","Create content, share ideas and build your presence.","/ideas","Meet ANVYA"]].map(([icon,title,desc,href,label])=><div key={title} className="rounded-3xl border bg-card p-7"><div className="text-3xl">{icon}</div><h3 className="mt-4 text-2xl font-black">{title.replace(/^[^ ]+ /,"")}</h3><p className="mt-2 font-semibold">{desc}</p><p className="mt-3 text-sm leading-7 text-muted-foreground">{href === "/services" ? "SEO, AI SEO, AEO, GEO, digital marketing, automation and AI technology solutions." : desc}</p><Button asChild variant="outline" className="mt-6"><Link to={href}>{label} <ArrowRight className="ml-2 h-4 w-4" /></Link></Button></div>)}
      </div></div></section>

      <section className="py-20"><div className="container mx-auto max-w-5xl px-4"><div className="rounded-[2rem] glass-card glass-sheen p-8 text-center md:p-12"><Bot className="mx-auto h-10 w-10 text-primary" /><h2 className="mt-5 text-3xl font-black md:text-5xl">🚀 Your Digital Journey Starts Here</h2><div className="mx-auto mt-7 max-w-2xl space-y-3 text-left text-muted-foreground"><p><strong className="text-foreground">Want more visibility?</strong> → Explore SEO & AI Services</p><p><strong className="text-foreground">Want to promote something?</strong> → Post a Product or Service</p><p><strong className="text-foreground">Want to create something?</strong> → Meet ANVYA & Start Creating</p><p><strong className="text-foreground">Want to learn?</strong> → Explore the Crazy SEO Team Blog</p></div><p className="mt-8 text-lg font-black">Build Your Presence. Share Your Ideas. Get Discovered.</p><p className="mx-auto mt-4 max-w-3xl leading-8 text-muted-foreground">Crazy SEO Team connects search, AI, businesses, products and creators in one modern digital ecosystem.</p><div className="mt-8 flex flex-wrap justify-center gap-4"><Button asChild className="gradient-cta text-primary-foreground"><Link to="/services">Explore SEO & AI Services</Link></Button><Button asChild variant="outline"><Link to="/ideas">Meet ANVYA</Link></Button></div></div></div></section>
    </main>
    <Footer />
    <WhatsAppButton />
  </div>
