...
export default function PunchClock({ onAttendanceChange }: { onAttendanceChange?: (attendance: Attendance | null) => void }) {
  ...
  const handlePunchIn = async () => {
    setBusy(true);
    try { applyAttendance(await punchIn()); toast.success("Punch In successful — CRM unlocked"); }
  ...
  const handlePunchOut = async () => {
    ...
    setBusy(true);
    try { applyAttendance(await punchOut(attendance)); toast.success("Punch Out recorded — CRM locked"); }
  ...
  return (
    <div className="flex items-center gap-2 rounded-2xl border border-border/70 bg-card/80 px-2.5 py-1.5 shadow-sm">
      ...
      {active ? <Button size="sm" variant="destructive" onClick={handlePunchOut} disabled={busy} className="rounded-xl h-8 px-3 gap-1.5"><LogOut size={14} /> Punch Out</Button> : <Button size="sm" onClick={handlePunchIn} disabled={busy} className="rounded-xl h-8 px-3 gap-1.5"><LogIn size={14} /> Punch In</Button>}
