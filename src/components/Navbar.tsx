...
const GaneshIcon = () => (
  ...
  >
    ...
    <path
      d="M20 27c-6-7-12-4-12 2 0 6 5 9 11 6M44 27c6-7 12-4 12 2 0 6-5 9-11 6"
      fill="none"
      stroke="currentColor"
      strokeWidth="3.5"
      strokeLinecap="round"
    />
    <path
      d="M23 25c1-8 6-12 9-12s8 4 9 12v10c0 8-5 14-9 14s-9-6-9-14z"
      fill="none"
      stroke="currentColor"
      strokeWidth="3.5"
    ...
    <path
      d="M27 22c2 2 8 2 10 0M26 29h2M36 29h2"
      fill="none"
      stroke="currentColor"
      strokeWidth="2.5"
      strokeLinecap="round"
      ...
      <nav className="cst-public-nav fixed top-0 left-0 right-0 z-50">
        ...
        <div className="cst-nav-inner">
          <div className="container mx-auto flex h-[68px] items-center justify-between px-4 lg:px-6">
            <Link id="tour-logo" to="/" className="group flex items-center gap-3">
              <div className="cst-brand-mark relative shrink-0">
                ...
                <img
                  src={logo}
                  alt="Crazy SEO Team"
                  className="relative h-10 w-10 rounded-xl object-contain ring-1 ring-slate-200/80 bg-white"
                />
              </div>
            ...
            <div
              id="tour-nav"
              className="cst-nav-pill hidden items-center gap-0.5 rounded-full border border-slate-200/80 bg-slate-50/85 p-1 lg:flex"
            >
              {navLinks.map((link) =>
                link.label === "Tools" ? (
                  <div
                  ...
                  >
                    <button
                      ...
                      className={`cst-nav-link inline-flex items-center gap-1 rounded-full px-3 py-2 text-[13px] font-semibold transition-all ${
                        toolsActive
                      ...
                      <div className="absolute left-1/2 top-full w-56 -translate-x-1/2 pt-2">
                        <div className="rounded-2xl border border-slate-200 bg-white p-2 shadow-xl">
                          ...
                          >
                            ...
                            <span>
                              <span className="block">SEO Tools</span>
                              <span className="block text-[10px] font-medium text-slate-400">
                                Audit, AEO, GEO, NLP & more
                              </span>
                            </span>
                          </Link>
                          <Link
                  ...
                  <Link
                    key={link.label}
                    to={link.path}
                    className={`cst-nav-link rounded-full px-3 py-2 text-[13px] font-semibold transition-all ${
                      isActive(link.path)
                        ? "bg-white text-slate-950 shadow-sm"
                        : "text-slate-500 hover:bg-white/80 hover:text-slate-900"
                    }`}
            ...
            <div id="tour-cta" className="hidden items-center gap-2 md:flex">
              <button
                type="button"
                className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-semibold text-slate-500 transition hover:bg-slate-100 hover:text-slate-900"
              >
                <Globe2 size={15} />
                Global
              </button>
              <Button
                ...
                className="rounded-xl border-slate-200 bg-white/80 text-slate-800 hover:bg-slate-50"
              >
                Talk to us
              </Button>
              <Button
                onClick={() => setDialogOpen(true)}
                className="group rounded-xl bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 px-4 text-white"
              >
                Start Free Audit <ArrowUpRight size={15} />
              </Button>
            </div>
              ...
              <div className="grid gap-1">
                {navLinks.map((link) =>
                  link.label === "Tools" ? (
                    <div
                      key={link.label}
                      className="rounded-xl border border-slate-100 bg-slate-50/60 p-1"
                    >
                      <button
                        type="button"
                        onClick={() => setToolsOpen((value) => !value)}
                        className="flex w-full items-center justify-between rounded-lg px-3 py-3 text-sm font-semibold text-slate-600"
                      >
                        Tools
                        <ChevronDown
                          size={16}
