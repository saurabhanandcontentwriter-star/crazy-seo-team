...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border">
        ...
        {open && (
          <div className="md:hidden bg-background border-b border-border px-4 pb-4 space-y-3">
            {navLinks.map((link) => (
              <a key={link.label} href={link.href} className="block text-sm font-medium text-muted-foreground hover:text-foreground" onClick={() => setOpen(false)}>
                {link.label}
              </a>
            ))}
      ...
      <ContactFormDialog open={dialogOpen} onOpenChange={setDialogOpen} title="Get Your Free SEO Audit" description="Enter your details and our AI will analyze your website's SEO performance." />
    </>
  );
};
