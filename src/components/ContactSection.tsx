...
import { Mail, Phone, MessageCircle, CalendarClock, MapPin, ArrowRight } from "lucide-react";
          ...
          <div className="relative grid gap-10 lg:grid-cols-2 lg:items-center">
            ...
            >
              <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1 text-[11px] font-semibold text-emerald-700">
                <MapPin size={11} /> Working with brands worldwide
              </span>
              ...
              <p className="mt-4 text-slate-600 leading-relaxed">
                Tell us your goals and we'll come back within 24 hours with a custom plan covering
              ...
              >
                Start free audit <ArrowRight size={16} className="ml-2" />
              </Button>
            </motion.div>
              ...
              {channels.map((c) => {
                const inner = (
                  <>
                    ...
                    <div className="mt-0.5 text-xs text-slate-600">{c.value}</div>
                  </>
                );
                const cls =
                ...
                return c.href ? (
                  ...
                  <button key={c.label} type="button" onClick={() => setOpen(true)} className={cls}>
                    {inner}
                  </button>
                );
              })}
